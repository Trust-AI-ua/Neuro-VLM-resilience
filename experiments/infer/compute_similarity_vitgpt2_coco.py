#!/usr/bin/env python3
"""Compute ViT-GPT2 caption similarity against COCO references (MPNet + ROUGE-2)."""
...
def main():
    ...
    ap.add_argument("--captions_json", required=True)  # member.json or nonmember.json
    ...
    # Support BLIP schema (caption only, no refs inside JSON).
    pairs = []
    missing = 0
    for it in items:
        s = stem_for(it)
        # BLIP uses "caption"; other models may use "pred"
        hyp = (it.get("caption") or it.get("pred") or "").strip()

        # check if reference captions exist in COCO
        if not hyp:
            missing += 1
            continue
        if s not in refs:
            # try stripping ".jpg" or extra prefixes from image_id if needed
            s_alt = s.replace(".jpg", "")
            if s_alt in refs:
                s = s_alt
            else:
    ...
    if not pairs:
        ...
        append_summary_row("COCO", "ViT-GPT2", args.tau, args.set, "MPNet_max", 0.0, 0)
        append_summary_row("COCO", "ViT-GPT2", args.tau, args.set, "ROUGE2_F1_max", 0.0, 0)
        return
    ...
    print(f"[COCO · ViT-GPT2 · τ={args.tau}] {args.set}: "
          f"MPNet_max={mp_mean:.4f}, ROUGE2_F1_max={r2_mean:.4f} (n={n}, missing={missing})")
    ...
    for i, s in enumerate(stems):
        ...
        out.append({
            "image_id": s + ".jpg",
            "ref": "<max-over-COCO-refs>",  #can remove if preferred
            temp_key: {
    ...
    print(f"[COCO · ViT-GPT2 · τ={args.tau}] {args.set}: MPNet_max={mp_mean:.4f}, ROUGE2_F1_max={r2_mean:.4f} (n={n}, missing={missing})")

    append_summary_row("COCO","ViT-GPT2",args.tau,args.set,"MPNet_max",mp_mean,n)
    append_summary_row("COCO","ViT-GPT2",args.tau,args.set,"ROUGE2_F1_max",r2_mean,n)
