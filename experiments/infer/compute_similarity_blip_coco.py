#!/usr/bin/env python3
"""Compute BLIP caption similarity against COCO references (MPNet + ROUGE-2)."""
...
def main():
    ...
    if not pairs:
        ...
        append_summary_row(DATASET, MODEL, args.tau, args.set, "MPNet_max", 0.0, 0)
        append_summary_row(DATASET, MODEL, args.tau, args.set, "ROUGE2_F1_max", 0.0, 0)
        return
    ...
    print(f"[{DATASET} · {MODEL} · τ={args.tau}] {args.set}: MPNet_max={mp_mean:.4f}, ROUGE2_F1_max={r2_mean:.4f} (n={n}, missing={missing})")
    ...
    for i, s in enumerate(stems):
        ...
        out.append({
            "image_id": s + ".jpg",
            "ref": "<max-over-COCO-refs>",  # optional; can remove
            temp_key: {
    ...
    print(f"[COCO · BLIP · τ={args.tau}] {args.set}: MPNet_max={mp_mean:.4f}, ROUGE2_F1_max={r2_mean:.4f} (n={n}, missing={missing})")
    append_summary_row("COCO","BLIP",args.tau,args.set,"MPNet_max",mp_mean,n)
    append_summary_row("COCO","BLIP",args.tau,args.set,"ROUGE2_F1_max",r2_mean,n)
